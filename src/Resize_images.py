...
class Collector: 
    ...
    def collect_data(self): 
        ...
        dataset_train = radarDataset_resized(root_dir, event_times, self.obs_time, self.pred_time, self.time_interval, transform = None)
        ...
        dataset_train_del = radarDataset_resized(root_dir, event_times, self.obs_time, self.pred_time, self.time_interval, transform = None)
        ...
        dataset_test = radarDataset_resized(root_dir, event_times,self.obs_time, self.pred_time, self.time_interval, transform = None)
        ...
        dataset_vali = radarDataset_resized(root_dir, event_times, self.obs_time, self.pred_time, self.time_interval, transform = None)
        ...
        dataset_train_aa = radarDataset_resized(root_dir, event_times,self.obs_time, self.pred_time, self.time_interval, transform = None)
        ...
        dataset_train_dw = radarDataset_resized(root_dir, event_times, self.obs_time, self.pred_time, self.time_interval, transform = None)
        ...
        dataset_train_re = radarDataset_resized(root_dir, event_times, self.obs_time, self.pred_time, self.time_interval, transform = None)
...
if __name__ == "__main__":
        root_dir = '/home/hbi/RAD_NL25_RAP_5min/' 
        obs_time, pred_time, time_interval = 4, 12, 30

        df_train = pd.read_csv('/users/ankushroy/taming-transformers/training_Delfland08-14_20.csv', header=None)
        event_times = df_train[0].to_list()
        dataset_train = radarDataset_resized(root_dir, event_times, obs_time, pred_time, time_interval, transform = None)

        df_train_s = pd.read_csv('/users/ankushroy/taming-transformers/training_Delfland08-14.csv', header=None)
        event_times = df_train_s[0].to_list()
        dataset_train_del = radarDataset_resized(root_dir, event_times, obs_time, pred_time, time_interval, transform = None)

        df_test = pd.read_csv('/users/ankushroy/taming-transformers/testing_Delfland18-20.csv', header=None)
        event_times = df_test[0].to_list()
        dataset_test = radarDataset_resized(root_dir, event_times,obs_time, pred_time, time_interval, transform = None)

        df_vali = pd.read_csv('/users/ankushroy/taming-transformers/validation_Delfland15-17.csv', header=None)
        event_times = df_vali[0].to_list()
        dataset_vali = radarDataset_resized(root_dir, event_times, obs_time, pred_time, time_interval, transform = None)

        df_train_aa = pd.read_csv('/users/ankushroy/taming-transformers/training_Aa08-14.csv', header=None)
        event_times = df_train_aa[0].to_list()
        dataset_train_aa = radarDataset_resized(root_dir, event_times,obs_time, pred_time, time_interval, transform = None)

        df_train_dw = pd.read_csv('/users/ankushroy/taming-transformers/training_Dwar08-14.csv', header=None)
        event_times = df_train_dw[0].to_list()
        dataset_train_dw = radarDataset_resized(root_dir, event_times, obs_time, pred_time, time_interval, transform = None)

        df_train_re = pd.read_csv('/users/ankushroy/taming-transformers/training_Regge08-14.csv', header=None)
        event_times = df_train_re[0].to_list()
        dataset_train_re = radarDataset_resized(root_dir, event_times, obs_time, pred_time, time_interval, transform = None)

        data_list = [dataset_train_aa, dataset_train_dw, dataset_train_del, dataset_train_re]
        train_aadedwre = torch.utils.data.ConcatDataset(data_list)

        dataloader_train= DataLoader(train_aadedwre, batch_size=64, shuffle=False, num_workers=8)
        dataloader_test = DataLoader(dataset_test, batch_size=64, shuffle=False, num_workers=8)
        dataloader_val = DataLoader(dataset_vali, batch_size=64, shuffle=False, num_workers=8)

        all_data_vali =[]
        image_list_v = [img.numpy() for img in dataset_vali]
        all_data_vali = np.stack(image_list_v)
        np.save('/space/ankushroy/all_data_vali.npy', all_data_vali)
        print(all_data_vali.shape)
        
        all_data_test =[]
        image_list_t = [img.numpy() for img in dataset_test]
        all_data_test = np.stack(image_list_t)
        np.save('/space/ankushroy/all_data_test.npy', all_data_test)
        print(all_data_test.shape)

        all_data_train =[]
        image_list_t = [img.numpy() for img in train_aadedwre]
        all_data_train = np.stack(image_list_t)
        np.save('/space/ankushroy/all_data_train.npy', all_data_train)
